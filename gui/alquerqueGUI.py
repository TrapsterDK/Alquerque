...
def get_params() -> None:
    ...
    white_AI = False
    return
    messagebox.showinfo("Hello!", "Welcome to Alquerque!")

    white_AI = messagebox.askyesno(
        "Before we start...", "Should the computer play white?"
    )
    black_AI = messagebox.askyesno(
        "Before we start...", "Should the computer play black?"
    )
    if white_AI or black_AI:
        AI_level = 0
        while not 1 <= AI_level <= 7:
            AI_level = simpledialog.askinteger(
                "Before we start...", "How smart should the computer be (1-7)?"
            )
            if not 1 <= AI_level <= 7:
                messagebox.showerror(
                    "Before we start...", "Please enter a number in the range 1-7."
                )
    messagebox.showwarning("Ready!", "The game will now start!")
...
def make_button(i: int, j: int, label: int) -> tk.Button:
    ...
    button = tk.Button(master=frame, text=label, image=img_empty)
    button.pack()
    return button
...
def make_separator(i: int, j: int, img: tk.PhotoImage) -> tk.Button:
    ...
    frame = tk.Frame(master=root)
    frame.grid(row=i, column=j)
    tk.Label(master=frame, image=img, fg="white").pack()
